...
void setup() {
  ...
  digitalWrite(ledR, LOW);                                   //  LED off initialisation (LOW - off, HIGH - on)
...
void loop() {
  ...
  if (!digitalRead(chargingPin)) {                                             // charging mode
    ...
    else if (hallSens && speedArr[2])  {                     motor.writeMicroseconds(motorStop); //start on 2 speed
